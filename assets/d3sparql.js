...
d3sparql.graph = function (json, config) {
    ...
    for (var i = 0; i < data.length; i++) {
        var key1 = data[i][opts.key1].value.slice(28);
        var key2 = data[i][opts.key2].value.slice(28);
        var label1 = data[i][opts.key1].value;
        var label2 = data[i][opts.key2].value;
        var value1 = data[i][opts.key1].value;
        var value2 = data[i][opts.key2].value;
        if (!check.has(key1)) {
        ...
        graph.links.push({ "source": 0, "target": check.get(key2) });
    }
    if (d3sparql.debug) {
...
d3sparql.forcegraph = function (json, config) {
    ...
    var svg = d3sparql.select(opts.selector, "forcegraph").append("svg")
        .attr("viewBox", "-533.5 -613.5 1280 1227")
        .attr("class", "result_data");
...
d3sparql.sunburst = function (json, config) {
    ...
    function arcTween(d) {
        var xd = d3.interpolate(x.domain(), [d.x, d.x + d.dx]), yd = d3.interpolate(y.domain(), [d.y, maxDepth(d)]), yr = d3.interpolate(y.range(), [d.y ? 20 : 0, radius]);
        return function (d) {
            return function (t) {
                x.domain(xd(t));
...
d3sparql.circlepack = function (json, config) {
    ...
    var w = opts.width, h = opts.height, r = opts.diameter, x = d3.scale.linear().range([0, r]), y = d3.scale.linear().range([0, r]);
